...
UsdAppUtilsFrameRecorder::UsdAppUtilsFrameRecorder(const TfToken& rendererPluginId, bool gpuEnabled)
    ...
    _imagingEngine.SetRendererSetting(HdRenderSettingsTokens->enableInteractive, VtValue(false));
}

void UsdAppUtilsFrameRecorder::SetActiveRenderSettingsPrimPath(SdfPath const& path) {
...
void UsdAppUtilsFrameRecorder::SetActiveRenderPassPrimPath(SdfPath const& path) {
...
void UsdAppUtilsFrameRecorder::SetColorCorrectionMode(const TfToken& colorCorrectionMode) {
...
void UsdAppUtilsFrameRecorder::SetCameraLightEnabled(bool cameraLightEnabled) {
    _cameraLightEnabled = cameraLightEnabled;
}

void UsdAppUtilsFrameRecorder::SetDomeLightVisibility(bool domeLightsVisible) {
...
public:
    TextureBufferWriter(RuntimeEngine* engine) : _engine(engine), _colorRenderBuffer(nullptr) {
...
private:
    bool _ValidSource() const { return _colorTextureHandle || _colorRenderBuffer; }
    ...
    HioFormat _GetFormat() const {
        if (_colorTextureHandle) {
            ...
            return HdStHioConversions::GetHioFormat(_colorRenderBuffer->GetFormat());
        } else {
    ...
    void* _Map() {
        if (_colorTextureHandle) {
            size_t size = 0;
            _mappedColorTextureBuffer =
                    HdStTextureUtils::HgiTextureReadback(_engine->GetHgi(), _colorTextureHandle, &size);
...
static bool _RenderProductsGenerated(const UsdStagePtr& stage, const SdfPath& renderSettingsPrimPath) {
    if (renderSettingsPrimPath.IsEmpty()) {
        return false;
    }

    bool productsGenerated = false;
    UsdRenderSettings settings = UsdRenderSettings(stage->GetPrimAtPath(renderSettingsPrimPath));
    ...
    for (const auto& productPath : renderProductTargets) {
        UsdRenderProduct product = UsdRenderProduct(stage->GetPrimAtPath(productPath));
        TfToken productName;
        product.GetProductNameAttr().Get(&productName);
        if (ArchOpenFile(productName.GetText(), "r")) {
            TF_STATUS(
                    "Product '%s' generated from RenderProduct prim <%s> "
                    "on RenderSettings <%s>",
                    productName.GetText(), productPath.GetText(), renderSettingsPrimPath.GetText());
            productsGenerated |= true;
        } else {
            TF_WARN("Missing generated Product '%s' from RenderProduct prim "
                    "<%s> on RenderSettings <%s>",
                    productName.GetText(), productPath.GetText(), renderSettingsPrimPath.GetText());
        }
    }
...
bool UsdAppUtilsFrameRecorder::Record(const UsdStagePtr& stage,
    ...
    const size_t imageHeight = std::max<size_t>(static_cast<size_t>(static_cast<float>(_imageWidth) / aspectRatio), 1u);
    ...
    _imagingEngine.SetRendererSetting(HdRenderSettingsTokens->domeLightCameraVisibility, VtValue(_domeLightsVisible));
